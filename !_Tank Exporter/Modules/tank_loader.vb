...
#End Region
    ...
    Public Function build_primitive_data(ByVal _add As Boolean) As Boolean
        ...
        '------------
        Dim jj As UInt32
        Dim section_addresses(100) As ULong
        ...
        '
        ' WoTs files are a total mess.
        ' Nothing remains constant in ANY file or file name.
        ' upper and lower case used for the same attribute names
        ' order of data inconsistent.
        ' headings for sections when there is no data after. (ctree files)
        ' many tank models with a uv2 stream.. others with color.. (WTF??!!)
        xmlget_mode = 0
        ' 1 = Hull
        ' 2 = Chassis
        ' 3 = Turret
        ...
        Try
            ...
            For Each p In p_
                ...
                ordered_names(cnt).vert_name = p.vert_name
                ordered_names(cnt).indi_name = p.indi_name
                If p.uv2_name IsNot Nothing Then
                    If Not p.uv2_name.Contains("colour") Then
                        ordered_names(cnt).uv2_name = p.uv2_name
                    End If
                End If
                If p.colour_name IsNot Nothing Then
                    If Not p.colour_name.Contains("uv2") Then
                        ordered_names(cnt).color_name = p.colour_name
                    End If
                End If
                    ...
                    For Each pa In pp_
                        If pa IsNot Nothing Then
                            If pa.Contains("colour") Then
                                ordered_names(cnt).color_name = pa
                            Else
                                If pa.Contains("uv2") Then
                                    ordered_names(cnt).uv2_name = pa
                                End If
                            End If
                        End If
                    Next
        ...
        ReDim Preserve section_names(100)
        For i = 0 To 99
...
#End If
        ...
        While sub_groups > 0
            ...
            ih.nInd_groups = ib_reader.ReadUInt16
